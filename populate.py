...
def get_radius_logs():
    ...
    isotime = time_5mins.astimezone(central).isoformat()

    data= {"startTimestamp": isotime}
    ...
    response = requests.request("POST", url, headers=headers,  data=json.dumps(data),auth = HTTPBasicAuth(config.pxgrid_user, config.pxgrid_secret),verify=False)
...
def collect():
    ...
    for radius_log in radius_logs['sessions']:
        try:
            date_time = radius_log["timestamp"].split(".")[0]
            print(date_time)
            
            utc_datetime = datetime.datetime.strptime(date_time, '%Y-%m-%dT%H:%M:%S')
            local_timezone = pytz.timezone("US/Central")
            local_datetime = utc_datetime.replace(tzinfo=pytz.utc)
            local_datetime = local_datetime.astimezone(local_timezone)
            #date_time = str(local_datetime).split("-05:00")[0]
            date_time = str(local_datetime).rsplit("-", 1)[0]
